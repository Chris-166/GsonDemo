package com.example.fanff.gsondemo01;
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btn_person:
                ...
                Person person = GsonTools.getPerson(jsonContent, Person.class);// 解析json字符串
            ...
            case R.id.btn_persons:
                ...
                List<Person> persons = GsonTools.getPersons(jsonContent, Person.class);
                System.out.println(persons.toString());
                break;
            case R.id.btn_listString:
                url_path = path + "?flag=liststring";
                jsonContent = HttpUtils.getJsonContent(url_path);
                List<String> list = GsonTools.getList(jsonContent);
                System.out.println(list.toString());
                break;
            case R.id.btn_listmap:
                url_path = path + "?flag=listmap";
                jsonContent = HttpUtils.getJsonContent(url_path);
                List<Map<String, Object>> listmap = GsonTools.listKeyMaps(jsonContent);
